...
class LoggerController extends Controller {
  ...
  _getInfo() {
    return _.extend({
      ...
      host: os.hostname(),
      service: _.get(this, 'context.name')
  ...
  _log(level, args) {
    ...
    if (configNumberLevel >= logNumberLevel) {
      ...
      return Promise.try(() => this.service.log(
        info,
        parsedArgs.msg,
        _.extend(parsedArgs.context, this.options.context),
        parsedArgs.error
      )).catch(err => {
        console.error("Cannot log message", err);
      });
    }
  }

  create(name, options) {
    options = _.extend(_.clone(this.options), options, {component: name});
